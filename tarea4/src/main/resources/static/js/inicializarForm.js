...
for (const checkbox of checkboxes){
    ...
    checkbox.onchange = () => {
        let opt = checkbox.name
        let divname = checkbox.id.slice(-5)
        if (checkbox.checked) {
            ...
            document.getElementById(divname).appendChild(inp)
        } else{
            document.getElementById(divname).removeChild(document.getElementById(opt + "-input"))
        }
    }
}
...
addPhotoBtn.onclick = () => {
    ...
    let inp = document.createElement("input")
    inp.type = "file"
    inp.name = "foto"+(1+nFotos).toString()
